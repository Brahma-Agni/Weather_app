...
const CustomWeatherDataSchema = z.object({
  location: z.string(),
...
export async function getCustomWeatherData(data: z.infer<typeof CustomWeatherDataSchema>) {
  // In a real app, you would fetch this from a weather API based on location and date.
  ...
  const analysisPromises = [
    analyzeForecastProbabilities({
      ...
      userPlans: `Possible activities in ${data.location}.`,
      date: data.date,
    }),
    getWeatherRiskScore({
...
export type WeatherResult = Awaited<ReturnType<typeof getCustomWeatherData>>;
